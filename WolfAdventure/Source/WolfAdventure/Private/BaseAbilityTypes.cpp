...
{
	// we use this to know whether to serialize or not (1 = yes, 0 = no) a given variable determined by the bit position
	uint32 RepBits = 0;
	if (Ar.IsSaving())
	{
	...
	if (RepBits & (1 << 14))
	{
		// this is how we serialize a vector so it can be replicated down to the pipeline and reach the clients through the net 
